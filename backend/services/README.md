...
`backend/services` 存放 `rfpt` 后端业务服务模块。新增或修改服务代码前，先读取：
...
- `rfpt-mng`：管理端服务，承载管理后台 API、社保缴费管理、员工绩效管理入口。
- `rfpt-performance`：员工绩效服务，承载绩效任务、员工绩效记录、H5 登录、确认、反馈、自动确认和后台调整能力。
